...
// jika user sudah login, maka jalankan perintah untuk ubah password
    ...
    if (isset($_POST['simpan'])) {
        if (isset($_SESSION['id_user'])) {
            ...
            $old_pass    = sha1(md5(mysqli_real_escape_string($mysqli, trim($_POST['old_pass']))));
            $new_pass    = sha1(md5(mysqli_real_escape_string($mysqli, trim($_POST['new_pass']))));
            $retype_pass = sha1(md5(mysqli_real_escape_string($mysqli, trim($_POST['retype_pass']))));

            // ambil data hasil session user
            $id_user = $_SESSION['id_user'];

            // seleksi password dari tabel user untuk dicek
            $sql = mysqli_query($mysqli, "SELECT password FROM sys_user WHERE id_user=$id_user")
            ...
            // jika input password lama sama dengan password didatabase, jalankan perintah untuk pengecekan selanjutnya
                ...
                // selain itu, jalankan perintah update password
                    ...
                    $query = mysqli_query($mysqli, "UPDATE sys_user SET password = '$new_pass'
